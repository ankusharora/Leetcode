...
class ExternalService {
    // Simulate a thread pool for async tasks (in a real app, this would be managed externally)
    private static final ExecutorService executor = Executors.newFixedThreadPool(4);

    public static CompletableFuture<String> fetchDeliveriesJsonAsync() {
        return CompletableFuture.supplyAsync(() -> {
            // Simulate a slow provider call (e.g., 2-second delay)
            try {
    ...
    // Optional: Shutdown the executor when the app closes
...
public class SolutionWithAsync {
    public static void main(String[] args) {
        try {
            // Start the async fetch
            CompletableFuture<String> jsonFuture = ExternalService.fetchDeliveriesJsonAsync();

            // Do other work while waiting (simulated here)
            System.out.println("Fetching data asynchronously... doing other work in the meantime.");

            // Get the result when ready (this blocks until complete, or you can chain further)
            String json = jsonFuture.get(); // Could use .join() for non-blocking in some contexts
            ...
            ExternalService.shutdown(); // Clean up executor
        }
    }
}